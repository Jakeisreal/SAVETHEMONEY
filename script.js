...
function skipOnboarding(){ state.onboardingShown = true; persistState(); document.getElementById('onboardingOverlay')?.classList.remove('active'); }function switchTab(event, tabName){
document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active'));
document.querySelectorAll('.tab-content').forEach(c=>c.classList.remove('active'));
if(event?.currentTarget) event.currentTarget.classList.add('active');
const targetContent = document.getElementById(tabName);
if(targetContent) targetContent.classList.add('active');if (tabName==='dashboard') renderDashboard();
if (tabName==='report') renderReport();
if (tabName==='settings') renderSettings();
if (tabName==='plan') renderPlanEntry();
if (tabName==='summary') renderSummary();updateProgressTracker();
}/* ===================== 저장/로드 ===================== */
function loadState(){
const defaults = {
...
travelPolicy: {
  ...
  perDiemRules: {
    ...
    perRankByBand: {
      '원거리': { '사원': 20000, '대리': 25000, '과·차 이상': 30000 } // 요청값. 필요시 화면에서 수정 가능.
    },
...
return        <div class="progress-step ${klass}">         <div class="progress-step-header">           <div class="progress-step-icon">${icon}</div>           <div class="progress-step-title">${st.title}</div>         </div>         <div class="progress-step-desc">${st.desc}</div>       </div>;
}).join('');
}
/* ===================== SETTINGS ===================== */
...
const rows = (S.leadershipLevels||[]).map(lv=>     <tr>       <td><input type="text" value="${escapeHtml(lv.level||'')}" onchange="updateLeadershipLevel('${lv.id}','level',this.value)"></td>       <td>         <input type="text" inputmode="numeric" value="${formatNumberInput(lv.unitCost||0)}" onchange="updateLeadershipLevel('${lv.id}','unitCost',this.value)">         ${hangulHintHtml(lv.unitCost||0)}       </td>       <td><button class="button button-tertiary" onclick="removeLeadershipLevel('${lv.id}')">삭제</button></td>     </tr>).join('');
return `
  <div class="section-card">
    <div class="section-header">
      <div class="description">
...
const rows = (S.jobSegments||[]).map(seg=>     <tr>       <td><input type="text" value="${escapeHtml(seg.name||'')}" onchange="updateJobSegment('${seg.id}','name',this.value)"></td>       <td><input type="text" inputmode="numeric" value="${formatNumberInput(seg.ratio||0)}" onchange="updateJobSegment('${seg.id}','ratio',this.value)"></td>       <td>         <input type="text" inputmode="numeric" value="${formatNumberInput(seg.unitCost||0)}" onchange="updateJobSegment('${seg.id}','unitCost',this.value)">         ${hangulHintHtml(seg.unitCost||0)}       </td>       <td>         <select onchange="updateJobSegment('${seg.id}','category',this.value)">           <option value="고객사" ${seg.category==='고객사'?'selected':''}>고객사</option>           <option value="고객사 외" ${seg.category!=='고객사'?'selected':''}>고객사 외</option>         </select>       </td>       <td><button class="button button-tertiary" onclick="removeJobSegment('${seg.id}')">삭제</button></td>     </tr>).join('');
return `
  <div class="section-card">
    <div class="section-header">
      <div class="description">
...
const rows = (S.legalTypes||[]).map(t=>     <tr>       <td><input type="text" value="${escapeHtml(t.type||'')}" onchange="updateLegalType('${t.id}','type',this.value)"></td>       <td><input type="text" inputmode="numeric" value="${formatNumberInput(t.ratio||0)}" onchange="updateLegalType('${t.id}','ratio',this.value)"></td>       <td>         <input type="text" inputmode="numeric" value="${formatNumberInput(t.unitCost||0)}" onchange="updateLegalType('${t.id}','unitCost',this.value)">         ${hangulHintHtml(t.unitCost||0)}       </td>       <td><button class="button button-tertiary" onclick="removeLegalType('${t.id}')">삭제</button></td>     </tr>).join('');return `
  <div class="section-card">
    <div class="section-header">
      <div class="description">
        <h2>⑤ 법정의무 교육</h2>
...
const tp = state.settings.travelPolicy;
const d = tp.destinations.find(x=>x.id===id); if(!d) return;
const prevName = d.name;if (field==='name'){
...
if (!r.perRankByBand['원거리']) r.perRankByBand['원거리'] = {};
...
const TEAM_RANK_OPTIONS = ['사원','대리','과·차 이상'];/* === Travel Policy 렌더 (교통비.xlsx 업로드 포함) === */
...
const r = tp.perDiemRules;return `
  <div class="section-card">
    <div class="section-header">
      <div class="description">
...
}function triggerTravelXlsx(){ document.getElementById('travelXlsxInput')?.click(); }/* 교통비.xlsx 핸들러
...
const rows = XLSX.utils.sheet_to_json(ws);
  const tp = state.settings.travelPolicy; initTravelMatrixIfNeeded(tp);  rows.forEach(r=>{
    const origin = String(r['출발지']||'').trim();
    const dest   = String(r['출장지역']||'').trim();
    if (!origin || !dest) return;    const car   = parseNumberInput(r['자차교통비(왕복)']) || 0;
    const publicT = parseNumberInput(r['대중교통비(왕복)']);
    const fare  = Number.isFinite(publicT) && publicT>0 ? publicT : car;    const nearFlag = String(r['근거리여부']||'').trim();
    const band = (nearFlag==='O' || nearFlag==='o' || nearFlag==='Y') ? '근거리' : '원거리';    // 출발지/목적지 목록에 없으면 추가
    if (!tp.origins.some(o=>o.name===origin)) tp.origins.push({ id:gen('org'), name:origin });
    if (!tp.destinations.some(d=>d.name===dest)) tp.destinations.push({ id:gen('dst'), name:dest, band });    if (!tp.matrix[origin]) tp.matrix[origin]={};
    if (!tp.bandMatrix[origin]) tp.bandMatrix[origin]={};    tp.matrix[origin][dest] = fare;
...
return        <tr>         <td><input type="text" value="${escapeHtml(t.name||'')}" onchange="updateTeam('${t.id}','name',this.value)"></td>         <td><input type="number" min="0" value="${t.headcount||0}" onchange="updateTeam('${t.id}','headcount',this.value)"></td>         <td><select onchange="updateTeam('${t.id}','origin',this.value)">${options(originOpts, t.origin||firstOriginName())}</select></td>         <td><select onchange="updateTeam('${t.id}','rankLevel',this.value)">${options(rankOpts, t.rankLevel||'사원')}</select></td>         <td><input type="number" min="0" max="100" step="1" value="${t.customerSharePct||0}" onchange="updateTeam('${t.id}','customerSharePct',this.value)"></td>         <td><select onchange="updateTeam('${t.id}','customerDestination',this.value)">${options(destOpts, t.customerDestination||firstDestinationName())}</select></td>         <td><select onchange="updateTeam('${t.id}','nonCustomerDestination',this.value)">${options(destOpts, t.nonCustomerDestination||firstDestinationName())}</select></td>         <td>           <input type="text" inputmode="numeric" value="${formatNumberInput(jobBudget)}"                  onfocus="this.value=this.value.replace(/,/g,'')"                  onblur="(function(el){                     const v = parseNumberInput(el.value) || 0;                     updateTeam('${t.id}','jobBudgetManual', v);                     el.value = formatNumberInput(v);                     const hint = el.parentElement.querySelector('.krw-hint');                     if (hint) hint.textContent = toKoreanCurrencyShort(v);                   })(this)">           <div class="krw-hint" style="font-size:.8em;color:#9cb5e9;margin-top:4px;">${toKoreanCurrencyShort(jobBudget)}</div>         </td>         <td><button class="button button-tertiary" onclick="removeTeam('${t.id}')">삭제</button></td>       </tr>;
  ...
  <div class="section-card">
    ...
    </div><div class="summary-table"><table>
  <thead><tr>
...
function removeTeam(id){ state.settings.teams = state.settings.teams.filter(t=>t.id!==id); persistState(); renderSettings(); renderSummary(); }/* Teams CSV/XLSX (주요 직급 포함) */
...
}function triggerTeamsUpload(){ document.getElementById('teamsFileInput')?.click(); }function handleTeamsFile(evt){
const file = evt.target.files?.[0]; if (!file) return;
const name = (file.name||'').toLowerCase();if (name.endsWith('.xlsx')){
...
try { cands.push({label:'euc-kr', dec: new TextDecoder('euc-kr', {fatal:false})}); } catch(){}
try { cands.push({label:'ms949',  dec: new TextDecoder('ms949',  {fatal:false})}); } catch(){}
...
el.innerHTML =      <div class="section-card">       <div class="section-header"><div class="description">         <h2>PLAN ENTRY</h2><p>트랙별 계획(인원×차수×단가)을 입력합니다.</p>       </div></div>       ${renderPlanTable('leadership','리더십',['계층','인원','차수','단가(원)'])}       ${renderPlanTable('job','직무',['과정명','인원','차수','가중평균단가(원)','기타비용(원)','참고'])}       ${renderPlanTable('hierarchy','직급공통',['직급','인원','차수','단가(원)'])}       ${renderPlanTable('legal','법정',['항목','인원','차수','단가(원)'])}       ${renderPlanTable('misc','기타',['항목','인원','차수','단가(원)','비고'])}     </div>;
...
if (key==='leadership') return <tr>     <td>${text(r.level, updatePlanField.bind(null,'key′,′{key}','
key′,′{r.id}','level')
)}</td>     <td>${num(r.headcount, updatePlanNumber.bind(null,'key′,′{key}','
key′,′{r.id}','headcount')
)}</td>     <td>${num(r.rounds, updatePlanNumber.bind(null,'key′,′{key}','
key′,′{r.id}','rounds')
)}</td>     <td>${num(r.unitCost, updatePlanNumber.bind(null,'key′,′{key}','
key′,′{r.id}','unitCost')
)}</td>     <td><button class="button button-tertiary" onclick="removePlanRow('${key}','${r.id}')">삭제</button></td></tr>;
if (key==='job') return <tr>     <td>${text(r.name, updatePlanField.bind(null,'key′,′{key}','
key′,′{r.id}','name')
)}</td>     <td>${num(r.headcount, updatePlanNumber.bind(null,'key′,′{key}','
key′,′{r.id}','headcount')
)}</td>     <td>${num(r.rounds, updatePlanNumber.bind(null,'key′,′{key}','
...
key′,′{r.id}','unitCost')
)}</td>     <td>${num(coalesce(r.otherCost, state.settings.jobDefaultOtherCost), updatePlanNumber.bind(null,'key′,′{key}','
key′,′{r.id}','otherCost')
)}</td>     <td class="note">팀 배분은 Settings▶Travel/Teams 사용</td>     <td><button class="button button-tertiary" onclick="removePlanRow('${key}','${r.id}')">삭제</button></td></tr>;
if (key==='hierarchy') return <tr>     <td>${text(r.level, updatePlanField.bind(null,'key′,′{key}','
key′,′{r.id}','level')
)}</td>     <td>${num(r.headcount, updatePlanNumber.bind(null,'key′,′{key}','
key′,′{r.id}','headcount')
)}</td>     <td>${num(r.rounds, updatePlanNumber.bind(null,'key′,′{key}','
...
if (key==='legal') return <tr>     <td>${text(r.type, updatePlanField.bind(null,'key′,′{key}','
key′,′{r.id}','type')
)}</td>     <td>${num(r.headcount, updatePlanNumber.bind(null,'key′,′{key}','
key′,′{r.id}','headcount')
)}</td>     <td>${num(r.rounds, updatePlanNumber.bind(null,'key′,′{key}','
...
return <tr>     <td>${text(r.name, updatePlanField.bind(null,'key′,′{key}','
key′,′{r.id}','name')
)}</td>     <td>${num(r.headcount, updatePlanNumber.bind(null,'key′,′{key}','
key′,′{r.id}','headcount')
)}</td>     <td>${num(r.rounds, updatePlanNumber.bind(null,'key′,′{key}','
...
key′,′{r.id}','unitCost')
)}</td>     <td>${text(r.note||'', updatePlanField.bind(null,'key′,′{key}','
key′,′{r.id}','note')
)}</td>     <td><button class="button button-tertiary" onclick="removePlanRow('${key}','${r.id}')">삭제</button></td></tr>;
...
function updatePlanNumber(key,id,field,value){ const row=(state.plans[key]||[]).find(r=>r.id===id); if(!row) return; row[field]=parseNumberInput(value)??0; persistState(); renderSummary(); renderDashboard(); renderReport(); }/* ===================== SUMMARY 계산 ===================== */
...
const raw = headroundsunit;
return { id:r.id, cost: roundWon(raw), head, rounds, unit };
});
return { items, total: sum(items.map(i=>i.cost)) };
}function weightedAverageUnitCost(segments){ return sum((segments||[]).map(s=>((+s.ratio||0)/100)*(+s.unitCost||0))); }function weightedAvgByCategory(segments){
...
const raw = (headrounds*unit) + other;
...
const rules = tp.perDiemRules;// 기본 Per-Diem
let perDiem = 0;
...
const rank  = team.rankLevel || '사원';// 수강료: 고객사 0, 비고객사만 적용
const tuitionNon = roundWon(head * sNon * unitNonCustomer);
const tuition = tuitionNon;// 출장비: 직급 반영(원거리)
...
const travel = roundWon(costC + costN);const total = roundWon(tuition + travel);return {
...
const line = (k,v,bold=false)=>     <tr${bold?' style="font-weight:700"':''}>       <td>${k}</td>       <td style="text-align:right">${formatNumberInput(v)} 원</td>       <td style="text-align:right;color:#6b7280">${toKoreanCurrencyShort(v)}</td>     </tr>;
return `
  <div class="section-card">
    <div class="section-header">
      <div class="description">
...
<p><b>팀별 자동 배분(안)</b>: 고객사 교육은 출장비만, 비고객사 교육은 교육비('고객사 외' 가중단가) + 출장비를 <b>차수</b>·<b>직급</b> 기준으로 계산합니다.</p>
  ...
  <div class="metrics-grid">
    ${Object.entries(totals.totalByTrack).map(([k,v])=>`
      <div class="metric-card"><h4>${labelOf(k)} 이액</h4><p>${formatNumberInput(v)} 원<br><span class="mini">${toKoreanCurrencyShort(v)}</span></p></div>`).join('')}
...
el.innerHTML =      <div class="section-card">       <div class="section-header"><div class="description">         <h2>DASHBOARD</h2><p>추후 차트 연동</p></div></div>       <div class="empty-placeholder">차트 구성 예정</div>     </div>;
}
function renderReport(){
const el = document.getElementById('reportContent'); if (!el) return;
el.innerHTML =      <div class="section-card">       <div class="section-header"><div class="description">         <h2>REPORT</h2><p>경영 보고용 요약.</p></div></div>       <div class="report-container">         <div class="report-title">교육비 사업계획(요약)</div>         <div class="report-subtitle">${state.settings.year||''}년 / 통화: ${escapeHtml(state.settings.currency||'KRW')}</div>         <div class="note">SUMMARY 기준.</div>       </div>     </div>;
}
/* ===================== window 바인딩 ===================== */
Object.assign(window, {
startOnboarding, skipOnboarding, showOnboarding, switchTab,
updateSetting,
